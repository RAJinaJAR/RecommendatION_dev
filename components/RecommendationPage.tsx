...
import { saveRecommendationRecord, saveLeadRecord } from '../services/storageService';
    ...
    return (
        <div className="mb-8 border border-ion-gray-medium rounded-lg">
            ...
            >
                ...
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={`transform transition-transform duration-300 ${isOpen ? 'rotate-180' : ''}`}><path d="m6 9 6 6 6-6"/></svg>
            </button>
            {isOpen && (
...
const ProductCard: React.FC<{ product: Product; tailoredDescription: string; }> = ({ product, tailoredDescription }) => (
    <div className="border border-ion-gray-medium rounded-xl p-6 shadow-lg bg-white flex flex-col h-full">
        <div className="text-center">
            <h2 className="text-3xl font-bold text-ion-blue">{product.name}</h2>
        </div>
        <p className="text-ion-gray-dark my-4 text-center">{product.description}</p>
        <div className="border-t border-ion-gray-medium pt-4">
            <h3 className="font-semibold text-lg text-gray-800 mb-3 text-center">Why it's a good fit for you</h3>
            <ul className="space-y-2">
                {tailoredDescription.trim().split('\n').map((point, index) => {
                     ...
                     return (
                        <li key={index} className="flex items-start">
                            ...
                            <span className="text-gray-700">{cleanedPoint}</span>
                        </li>
                     );
                })}
            </ul>
        </div>
        <div className="mt-auto border-t border-ion-gray-medium pt-4 mt-4 text-center">
             <h3 className="font-semibold text-lg text-gray-800 mb-2">Your Point of Contact</h3>
             <div>
                <p className="font-bold text-gray-800">{product.specialist.name}</p>
                <a href={`mailto:${product.specialist.email}`} className="text-ion-blue hover:underline">{product.specialist.email}</a>
             </div>
        </div>
    </div>
);
...
export const RecommendationPage: React.FC<RecommendationPageProps> = ({ answers, onRestart }) => {
  ...
  const [idealDescription, setIdealDescription] = useState<string>('');
  const [strongDescription, setStrongDescription] = useState<string>('');
  ...
  const [isLeadFormSubmitted, setIsLeadFormSubmitted] = useState<boolean>(false);
  ...
  useEffect(() => {
    const fetchRecommendations = async () => {
      ...
      const text = await generateComparisonText(answers, recs.ideal, recs.strong);
 
      // Parse the generated text to split it between the two products
      const strongProductTitle = `**${recs.strong.name}:**`;
      const parts = text.split(new RegExp(`\\s*${strongProductTitle.replace(/\*/g, '\\*')}\\s*`));
 
      const idealDesc = parts[0].replace(`**${recs.ideal.name}:**`, '').trim();
      setIdealDescription(idealDesc);
 
      const strongDesc = parts.length > 1 ? parts[1].trim() : '';
      setStrongDescription(strongDesc);
  ...
  const renderFeedbackSection = () => {
      switch(feedbackState) {
        case 'prompt':
          return (
             <>
              <p className="text-center text-ion-gray-dark mb-4">Did you find these recommendations accurate?</p>
    ...
    <div className="min-h-screen bg-ion-gray-light p-4 sm:p-8 animate-fade-in flex items-center justify-center">
      <div className="max-w-6xl w-full mx-auto bg-white rounded-xl shadow-2xl overflow-hidden">
        <div className="bg-ion-blue text-white p-8 text-center">
            <h1 className="text-3xl md:text-4xl font-bold">Your Top ION CTRM Recommendations</h1>
        </div>
        ...
        {isLoading || !recommendations ? (
            ...
            <div className="p-6 sm:p-8">
                <div className="grid md:grid-cols-2 gap-8 mb-8">
                    <ProductCard product={recommendations.ideal} tailoredDescription={idealDescription} />
                    <ProductCard product={recommendations.strong} tailoredDescription={strongDescription} />
                </div>
               
                <div className="bg-ion-gray-light p-6 rounded-lg my-8">
                  {isLeadFormSubmitted ? (
                     <div className="text-center p-6 bg-green-50 rounded-lg animate-fade-in">
                        <p className="text-green-600 font-bold text-xl">✅ Thank you for your interest!</p>
                        <p className="text-ion-gray-dark mt-1">Our team will be in touch with you shortly.</p>
                    </div>
                  ) : (
                    <LeadCaptureForm
                        recommendations={recommendations}
                        onSubmitted={() => setIsLeadFormSubmitted(true)}
                    />
                  )}
                </div>
